...
public class NormalTile extends Tile implements Serializable {
	...
	public void land(Rectangle2DCoord rect) {
		...
		board.refreshBoard();
		/*
		 * int playerNumber; PlayModeController pmc = new PlayModeController();
		 * Player player=game.getCurrentPlayer(); int
		 * gameIndex=TileOApplication.getTileO().indexOfGame(game)+1;
		 ...
		 * switch(player.getColorFullName()){ case "RED":
		 * circle.setColor(Color.RED); break; case "YELLOW":
		 * circle.setColor(Color.YELLOW); break; case "BLUE":
		 * circle.setColor(Color.BLUE); break; case "GREEN":
		 * circle.setColor(Color.GREEN); break; }
		 * System.out.println("Original player number"); if(playerNumber%4!=0){
		 * playerNumber=playerNumber-4; }
		 * System.out.println("The player number is " + playerNumber);
		 * System.out.println("The hashmap after put is " +
		 * playerTiles.keySet()); playerTiles.put(playerNumber, circle);
		 * System.out.println("The hashmap after put is " +
		 * playerTiles.keySet()); Tile t=boardTiles.get(rect);
		 * player.setCurrentTile(boardTiles.get(rect)); try {
		 * 
		 * pmc.land(t); if(game.getMode()==Game.Mode.GAME){ pmc.setNextPlayer();
		 * } System.out.println("Homie we made it");
		 * if(!visitedTiles.contains(rect)){ visitedTiles.add(rect); }
		 * TileOApplication.getGamePage().refresh(); pmc.save(); repaint(); }
		 * catch (Exception e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } }else{
		 * if(TileOApplication.getGamePage().flag){
		 * showMessage("Please roll die again"); }else{
